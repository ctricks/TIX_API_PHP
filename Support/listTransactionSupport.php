...
if($TixTransID <> '')
{
    $filter = ' where t.ID = ' . $TixTransID;
}

if($UserCreatedByID <> '')
{
    $filter = ' where c.CreatedByID = ' . $UserCreatedByID;
}

if($isActive <> '' and $TixTransID <> '')
{
    $filter .= ' and c.isActive = ' . $isActive;
}else
{
    $filter .= ' where c.isActive = ' . $isActive;
}

//$selectStatement= "Select * from " . $tableAffected . ' ' . $filter . " order by ID asc;";
$selectStatement= "select TicketID,t.Subject,t.Description,UserCommentID,u.username,Comment,Attachment,c.Remarks from tblsupportconvo c " .
                  "left join tblticket t on c.TicketID = t.ID " .
                  "left join tblusers u on c.UserCommentID = u.ID " . $filter .
